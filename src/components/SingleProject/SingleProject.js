...
const SingleProject = ({ deleteProject, projectDescription, src, title, tech, link, gh, instructions, index, id }) => {
  ...
  const allLinks = [deployLink, repoLink].filter((link) => link.href);

  const projectImg = deployLink.href ? <Link key={index + 'a'} src={src ? src : 'https://i.imgur.com/f0GwD3h.png'} location={deployLink.location} href={link} /> : <img className='non-link-img' src={src ? src : 'https://i.imgur.com/f0GwD3h.png'} alt={title} />;
  const description = (
  ...
  return (
    <section className={`single-project`}>
      {deleteProject && (
        <button className='delete-btn material-symbols-outlined' onClick={() => deleteProject(id)}>
          delete
        </button>
      )}
